...
| v3.9.11.4 | released 2026-08-03 | **[P3-26] v02 Global Sample Pool ships** (`pa_cli/sample_pool.py` 13 cmd_* + `pa sample-pool` group with 13 subcommands). 3 iron rules: user-write / Mavis-readonly / training-isolated. Layout: `~/.paper-agent/sample_pool/` (cross-platform user home). 5 sample libraries (P1-6/8/9/21) DEPRECATED → replaced by [P3-26]. 4 new gated tickets (P3-22/23/24/25) for future SoTA rerank. Phased data-gated plan: n=30/100/200/500. Honest 3-tier: v01 numbers are in-sample, do not generalize. Schema v1→v2 migration: relevance_labels.label nullable | 2026-08-03 |
| v3.9.11.5 | released 2026-08-10 | **`pa fetch` proxy port 7897 → 10808 documentation fix**: docstring + `--proxy` help text now reference actual V2Ray port (per 2026-08-06 port change). User confusion prevention. Doc-only | 2026-08-10 |
| v3.9.11.6 | released 2026-08-10 | **arXiv channel added** (`pa_cli/search.py` new `_search_arxiv()` via official `arxiv` PyPI). Prefer-arXiv routing fix in `pa_cli/fetch.py` for legacy `arxiv:NNNN.NNNNN` DOI prefix. Also DOI slug fix (replace `:` and other Windows-illegal chars). Channel cascade: arxiv → sci-hub → annas → direct. Real-corpus test passes | 2026-08-10 |
| v3.9.11.7 | released 2026-08-10 | **`fetch_doi` arXiv translation hotfix on top of v3.9.11.6**: legacy `arxiv:2310.06825` style input now correctly routes to arXiv channel (was failing because `_get_proxy_dict()` returned empty in subprocess context). Defensive URL escaping for all DOI inputs | 2026-08-10 |
| v3.9.11.8 | released 2026-08-10 | **PubMed as 7th default search engine** (`pa_cli/pubmed_channel.py`, NCBI E-utilities wrapper, no auth required, ~110 LOC). 36M biomedical citations. `?api_key=NCBI_API_KEY` optional for higher rate. Returns `pmid/title/journal/year/authors/doi/pub_types`. Default engine list now 7 (crossref, openalex, arxiv, semanticscholar, aminer, cnki, pubmed) | 2026-08-10 |
| v3.9.11.9 | released 2026-08-10 | **PubMed year filter post-filter hotfix on top of v3.9.11.8**: esearch `pdat` (online) vs `year` field (print) differ for epub-ahead-of-print papers. Fix: post-filter by year after esummary in `search_pubmed()`. ~10 LOC. Trade-off: count may drop below `limit`. 8/8 edge cases PASS | 2026-08-10 |
| v3.9.12.0 | released 2026-08-10 | **ClinicalTrials.gov as 8th default search engine** (`pa_cli/clinicaltrials_channel.py`, NCBI sister site, 500K+ trial registry records, free public API, no auth). Returns `nct_id/title/status/conditions/interventions/phase/enrollment/start_date/related_pubs`. 6/6 unit tests PASS. Companion to PubMed for "is anyone running a trial for this intervention?" question. PEDro engine DEFERRED (no public JSON API). GitHub tag `v3.9.12.0` created + release published (5.7KB notes) | 2026-08-10 |
| v3.9.13.0 | released 2026-08-14 | **TLS proxy hardening (Round 11)**: `_validate_proxy_security()` in `pa_cli/fetch.py` refuses remote HTTP/SOCKS5 proxy (HTTP proxy leaks target hostname in plaintext CONNECT). Local HTTP proxy on 127.0.0.1 warn+accept. Override `PAPER_AGENT_ALLOW_REMOTE_PROXY=1` for power users. 6/6 unit tests pass; end-to-end `pa fetch 10.1038/nature12373` → 943KB PDF + warning | 2026-08-14 |
...
| v3.9.17.0 | released 2026-08-18 | **[P3-28.1] `pa search-and-import` ships — closes the research workflow loop deferred from v3.9.16.0**: NEW `pa_cli/search_and_import.py` (22KB, 5 functions, top-level orchestrator). NEW CLI command `pa search-and-import --query X --project Y` (Click, ~120 LOC) runs 7 steps in one call: search (8 engines) → write temp Bibtex → fetch PDFs (cascade) → bucket downloaded/failed → push downloaded to library (idempotent) → create Zotero project if missing (idempotent) → add items + write master note (markdown table of downloaded + failed). 21 new tests in `test_output/test_search_and_import.py` (25KB, 21/21 pass, all mock-based — no live API needed). `pyzotero` + `pa fetch` + `pa search` + `pa jobs` + `pa zotero project` all reused (no new infra). **MINOR bump** (new top-level CLI command = significant additive surface). **Auth unchanged**: `$ZOTERO_API_KEY` + `$ZOTERO_LIBRARY_ID`. **Deferred to v3.9.17.1** ([P3-29.1]): `--with-obsidian` flag + auto-create matching Obsidian project + cross-link master note to Obsidian index. 1-line hint at end of successful run tells user the corresponding `pa obsidian project` commands. Same-name convention gives natural cross-referencing between Zotero collection and Obsidian project | 2026-08-18 |
| v3.9.17.1 | released 2026-08-18 | **[P2-17.1] PDF upload in `pa zotero push` + `pa search-and-import` ships (closes the metadata-only gap from v3.9.15.0)**: NEW `pa_cli.zotero_api.upload_pdfs()` function (linked_file + imported_file modes, supports batch + per-PDF error reporting). `push_items()` step 4 no longer `pass` — now scans `pdf_dir/{key}.pdf` and uploads matching PDFs as Zotero attachments via `pyzotero.attachment_simple()` (imported_file) or `item_template("attachment", linkmode="linked_file")` + `create_items()` (linked_file). `pa search-and-import` extended: `push_downloaded()` now passes `out_dir` as `pdf_dir` so downloaded PDFs auto-upload. 16 new tests in `test_output/test_zotero_upload_pdfs.py` (16KB, 16/16 pass, all mock-based). **No new dep** (uses existing pyzotero APIs). **PATCH bump** (`--pdf-dir` flag existed since v3.9.15.0; now it actually works). **Bug fix in same commit**: `pa_cli/zotero_api.py` `list_collection_notes()` was missing `out.sort + return out` (function had `out = []` + for-loop but no return; v3.9.16.0 tests passed by accident because mock returned iterable that triggered `for ... in` but function returned `None`); 2 tests in `test_zotero_collections` now pass. **Encoding cleanup** (also in same commit): multiple `pa_cli/*.py` had truncated UTF-8 em-dash sequences (0xE2 0x80 without 0x94) from earlier `PowerShell -replace` operations on em-dash characters; replaced with ASCII `--` via `test_output/_fix_truncated.py` (auto-fixer). 0 hits now for non-UTF8 sequences in `pa_cli/*.py` | 2026-08-18 |
| v3.9.17.2 | released 2026-08-18 | **[P3-29.1] `--with-obsidian` flag for `pa search-and-import` ships (closes the full Zotero + Obsidian bidirectional loop)**: NEW `pa_cli.search_and_import.setup_obsidian_project()` function — auto-creates matching Obsidian project (idempotent by slug) + adds a thought referencing the Zotero project + note keys + download counts. Graceful skip if `$PAPER_AGENT_OBSIDIAN_VAULT` env var unset (returns `status='skipped'`, no error). NEW Click flag `--with-obsidian/--no-obsidian` (default `--no-obsidian` — opt-in only to keep `pa search-and-import` behavior compatible for users who don't use Obsidian). CLI output updated: new `Obsidian project: created/exists (slug=..., thoughts=N)` + `Obsidian path: ...` lines. 11 new tests in `test_output/test_search_and_import_obsidian.py` (15KB, 11/11 pass, all mock-based). **No new dep** (reuses `pa_cli/obsidian.py` from v3.9.16.0). **PATCH bump** (single flag + single function, additive). **Closed loop**: 1 command (`pa search-and-import --query X --project Y --with-obsidian`) now runs 8 steps end-to-end: search + fetch + bucket + push + PDF upload + Zotero project + master note + Obsidian project + cross-ref thought. Both Zotero-touching + Obsidian-touching research workflow is now fully automated behind 1 opt-in flag | 2026-08-18 |
| v3.9.18.0 | released 2026-08-18 | **[P3-28.2] `pa zotero-project pull` / `export-bib` ship — bidirectional Zotero <-> local pa project**: NEW `pa_cli/zotero_api.py` 4 functions: `_zotero_type_to_bibtex_type` (24+ Zotero types → Bibtex, auto-detects mastersthesis from thesisType), `_zotero_creators_to_bibtex_author` (only `author` creatorType, handles first+last and org-name), `_sanitize_bibtex_key` (40-char safe cite-key), `zotero_item_to_bibtex` (single item → `@type{key,\n  ...\n}` string with cite-key from DOI/author+year/title, long abstract truncation, brace escaping), `collection_items_to_bibtex` (full collection with cite-key dedup `_2/_3` suffix + optional file output), `pull_collection_to_project` (full orchestrator: init pa project via `init_project()` from `pa_cli.project` + augment `meta.json` with `zotero_collection_key`/`name`/`version`/`source: "zotero-pull"` + write `refs.bib`). 2 new Click commands under `pa zotero-project`: `pull` (`--name`/`--key`/`--slug`/`--root`/`--overwrite`/`--json`) + `export-bib` (`--name`/`--key`/`--out`/`--json`). 47 new tests in `test_output/test_zotero_pull_export.py` (24KB, 47/47 pass, all mock-based — no live API needed). **No new dep** (reuses existing `pyzotero` + `pa_cli.project`). **MINOR bump** (2 new commands + 4 new functions + significant new surface). **Closed round-trip**: pull (Zotero → local) + push (local → Zotero, shipped v3.9.15.0) form a complete bidirectional sync. Use case: take a Zotero collection offline for `pa review` / `pa topics` / `pa search` analysis without losing Zotero-side authorship. Cross-link preserved: every Bibtex entry includes `zotero_key = {KEY}` for round-trip verification | 2026-08-18 |
| v3.9.19.0 | released 2026-08-18 | **[P3-28.3] `pa zotero-project diff` / `sync` ship — incremental Zotero → local updates (closes the workflow that makes the v3.9.18.0 round-trip useful)**: NEW `pa_cli/zotero_api.py` 3 functions: `_parse_refs_bib_dois(refs_bib_path)` (reuses `parse_bibtex_for_doi`; returns `{normalized_doi: bibtex_key}`), `diff_collection_to_local(client, collection_key, local_refs_bib_path)` (compare Zotero vs local refs.bib by normalized DOI; returns `{new_dois, removed_dois, new_items, zotero_n_items, local_n_dois, unchanged_n}`; **no per-item "updated" detection** in v3.9.19 — use `pa zotero-project pull --overwrite` for full refresh), `sync_collection_to_local(...)` (full orchestrator; default `dry_run=True`; with `dry_run=False` = `--apply`): appends NEW items to refs.bib in Bibtex format; KEEPS removed items locally (safety) but records them in `meta.json` under `removed_from_zotero: [dois]`; refreshes `zotero_collection_version` + `zotero_last_sync_at` + `n_items` in meta.json). 2 new Click commands under `pa zotero-project`: `diff` (--name/--key/--slug/--root/--json; shows new/removed/unchanged; first 20 of each list human-readable) + `sync` (--name/--key/--slug/--root/--apply/--no-apply/--json; default dry-run). 22 new tests in `test_output/test_zotero_diff_sync.py` (17KB, 22/22 pass). **No new dep**. **MINOR bump** (2 new commands + 3 new functions + new workflow). **Use case**: after adding 5 papers to a Zotero collection, run `pa zotero-project sync --apply` to update the local pa project refs.bib incrementally (instead of doing a full re-pull). meta.json `removed_from_zotero` list is the audit trail for items that disappeared from Zotero but are still in local. Idempotent: re-running `sync` after a previous `--apply` finds 0 new items | 2026-08-18 |
| v3.9.20.0 | released 2026-08-18 | **Research Workflow Suite: 4 new capabilities ship in 1 release — [P2-19] `pa project corpus-stats` + [P2-19.1] PRISMA in master note + [P3-29.2] `pa obsidian daily-link` + [P3-28.4] per-item update detection in `pa zotero-project diff/sync`**: 1 new module `pa_cli/corpus_stats.py` (6.4KB, stdlib only, `compute_corpus_stats(refs_bib_path, top_n=10)` + `format_corpus_stats_human`) computes n_papers/with-DOI/without-DOI/by-type/year-range+median/decade-histogram/top-authors/top-venues; `pa_cli/search_and_import.py` `_render_master_note` extended with `_render_prisma_block` (reuses P1-3 `pa_cli.prisma`) auto-embeds a mermaid flow diagram in every master note (identification→screening→eligibility→included); `pa_cli/obsidian.py` NEW `daily_link(project_name, date=None, vault_path=None, create_if_missing=False)` adds `## Active research projects` section to `<vault>/4-Daily/<date>.md` with wiki-link to `0-Research/Projects/<slug>/index` (idempotent via per-project HTML comment marker); `pa_cli/zotero_api.py` NEW `_parse_refs_bib_for_zotero_keys` (brace-counting parser, regex can't handle nested `title={X with } brace}`) + `get_collection_items` now includes `version` field + `diff_collection_to_local` extended with `local_meta_path` param + `updated_items`/`n_updated` output (compares Zotero `version` to `meta.json[zotero_item_versions][zk]`, only flags updates when a baseline exists) + `sync_collection_to_local` refreshes `zotero_item_versions` in meta.json on each apply. 3 new CLI commands: `pa project corpus-stats` (`--root`/`--top`/`--json`; no slug = all projects) + `pa obsidian daily-link` (`--project`/`--date`/`--vault`/`--create`/`--json`) + existing `pa zotero-project diff/sync` extended to show `updated: N` line. 50 new tests pass: `test_corpus_stats.py` (22/22) + `test_prisma_in_master_note.py` (4/4) + `test_obsidian_daily_link.py` (10/10) + `test_zotero_per_item_update.py` (14/14). **No new dep** (stdlib only for corpus-stats; reuses existing prisma/obsidian/zotero_api). **MINOR bump** (1 new module + 3 new commands + per-item update detection). 4 features in 1 release: corpus analytics, journal-ready PRISMA diagram in master note, daily-note backlinking, per-item edit detection in Zotero sync | 2026-08-18 |
| v3.9.20.1 | released 2026-08-21 | **`pa fetch` default auto mode was missing `pmc` channel (v3.9.20.1 PATCH fix)**: v3.9.21.0 added PMC fulltext channel to `pa_cli/fetch.py` but the default `--channels` list in `pa_cli/cli.py` was never updated to include `pmc`. Result: default auto mode for `pa fetch <DOI>` never tried PMC, even for papers 100% guaranteed in PMC (e.g. NIH-funded research). **Symptom**: `pa fetch 10.3389/fendo.2026.1798827` returned `ALL_FAIL` (sci-hub) when `pa fetch --prefer pmc 10.3389/fendo.2026.1798827` returned 16s SUCCESS via PMC. **Root cause**: `pa_cli/cli.py:227` default was `openalex,arxiv,unpaywall,doi_redirect,scihub,playwright` (no `pmc`); `fetch.py:1062` `elif "pmc" in channels: prefer = "pmc"` only triggers when `pmc` is in the channel list. **Fix**: default = `pmc,arxiv,openalex,unpaywall,doi_redirect,scihub,playwright` (PMC first — 100% legal, permanent, NIH/Europe PMC papers). **Verification**: same Frontiers paper 42s ALL_FAIL → 16s SUCCESS via `pmc_xml_only` (115KB JATS XML). 5 regression tests in `test_output/_test_pmc_default_fix.py` (5/5 pass). **Tradeoff** (later fixed in v3.9.21.0): PMC channel returned JATS XML saved with `.pdf` extension; for real PDF needed `scihub` or `unpaywall`. **PATCH bump** (1 line default + 1 line help + version bumps) |
| v3.9.21.0 | released 2026-08-21 | **[P3-30] JATS XML → real PDF pipeline ships (`pa_cli/jats_to_pdf.py`, 25KB, 16 functions, 746 lines)**: v3.9.20.1 trade-off "PMC returns JATS XML, not real PDF" closed. New module converts XML → HTML → Chromium PDF (Playwright): `jats_xml_to_html(xml_bytes, doi, pmcid)` (stdlib ET + HTML builder), `_render_figure(fig, depth, pmcid)` resolves relative `xlink:href` (e.g. `fendo-17-1798827-g001.jpg`) to `https://www.ncbi.nlm.nih.gov/pmc/articles/instance/{pmcid}/bin/<filename>` (NOT `/articles/PMC{id}/bin/...` which 404s), `_embed_figures_as_data_uris(html)` (urllib download + base64), `_html_to_pdf_via_playwright(html)` (Chromium render). Integrated into `pa_cli/fetch.py`: `_pmc_jats_to_pdf` helper + step 4 in `fetch_pmc_doi` (auto-fallback when Europe PMC PDF render fails) + `pmc-pdf` case in `fetch_doi` channel mapping. New CLI option `--prefer pmc-pdf` (forces PMC + jats_to_pdf, skip Europe PMC, 15-25s, always returns real PDF). Updated `pa_cli/cli.py` Choice + help text. **Why Playwright over weasyprint/reportlab**: weasyprint needs GTK native libs (missing), reportlab/fpdf2/xhtml2pdf not in deps, Playwright Chromium already a dep for CNKI. **Verification** (`10.3389/fendo.2026.1798827`): 115KB JATS XML → 310KB real PDF, %PDF-1.4 magic, 6 figures + 2 tables + 41 refs rendered. **17 new tests** in `test_output/_test_jats_to_pdf.py` (7.5KB): 3 helper, 3 regex, 7 HTML structure, 3 PDF — all pass in 47.3s. **MINOR bump** (new module + new CLI option + auto-integration in fetch cascade) |
...
| v3.9.23.0 | local 2026-08-27 (pending push) | **MINOR: Codex CLI Skill** — added `.agents/skills/paper-agent/` per the [open agent skills spec](https://developers.openai.com/codex/skills/) so paper-agent is installable as a standard Codex Skill (not just Python CLI). Skill contains: `SKILL.md` (frontmatter + when-to-trigger + 8 script docs + 3 workflows), `agents/openai.yaml` (Codex App UI metadata: display_name/policy/scripts), `scripts/` (8 deterministic Python wrappers: search/fetch/fetch-batch/review/citations/keys/cache/version — all return JSON to stdout, JSON error to stderr), `references/` (channels.md 14-channel reference, engines.md 7-engine reference, cli-cheatsheet.md quick reference). **Why**: User asked GPT to install paper-agent in Codex, GPT refused ("项目实际是 Python CLI/MCP 应用"). This release fixes that. **E2E verified** (6/8 scripts work, 2 fail due to expected missing-args): `version.py` (603B JSON), `search.py BERT` (BERT paper JSON), `citations.py 10.1038/nature12373 forward 3` (2401B JSON, Nature nanothermometry paper), `cache.py stats` (real cache JSON), `keys.py list` (1976B JSON), `fetch.py` (S2 API 429 dev IP — code OK), `review.py`/`fetch_batch.py` missing args (argparse errors, expected). **Tests**: 22/22 PASS in `test_output/_test_v3_9_23_0_skill.py` (script presence + --help + arg rejection + manifest validation). **Version**: 3.9.22.1 → 3.9.23.0 (MINOR because new public surface: Codex skill + 8 wrapper scripts). **Install**: `cp -r .agents/skills/paper-agent ~/.codex/skills/` for user-level, or to repo's `.agents/skills/` for repo-level |
| v3.9.23.1 | local 2026-08-27 (pending push) | **PATCH: Codex Skill 'No module named pa_cli' fix**. User reported the v3.9.23.0 skill fails with `No module named 'pa_cli'` when installed at `C:\Users\paper-agent-user\.codex\skills\paper-agent\`. Root cause: PA_ROOT was hardcoded as `Path(__file__).parent.parent.parent` (only works when skill is at `.agents/skills/paper-agent/` INSIDE the repo). Fix in 2 parts: **(1) `scripts/_pa_root.py`** — shared `find_pa_root()` with 4-strategy discovery (`$PAPER_AGENT_ROOT` env > `import pa_cli` > common paths > `pa` CLI on PATH). All 7 pa-dependent wrappers (search/fetch/fetch-batch/review/citations/keys/cache) updated to use it. New `pa_cli_not_found` error JSON with install hint, exit code 4. **(2) `scripts/bootstrap.py`** — auto-install script that runs `pip install -e <auto-detected-repo>` if pa_cli missing, verifies in fresh subprocess. **E2E verified**: bootstrap --check (pa_cli pre-installed) exit 0; bootstrap (auto-detect + install) exit 0 with "Successfully installed paper-agent-3.9.23.0"; search.py from C:\Users\paper-agent-user (unrelated cwd) returns 2589B BERT JSON, exit 0; _pa_root.py CLI mode prints "paper-agent root: G:\minimax - workspace\Paper agent". **Tests**: 30/30 PASS (extended from 22: added `TestPaRootDiscovery` 5 tests + `TestBootstrapScript` 3 tests). **SKILL.md**: added Installation section + find_pa_root() explanation. **Version bump**: SKILL frontmatter 3.9.23.0 → 3.9.23.1 (PATCH; pa_cli package version unchanged at 3.9.23.0 because pa_cli code itself didn't change). |
| v3.9.24.0 | local 2026-08-27 (pending push) | **MINOR: 4 user-reported search-quality fixes** (from Wilson disease end-to-end test). **Issue 1**: docs said 7 engines, actually 8 (ClinicalTrials.gov is the 8th, added v3.9.12.0). Updated SKILL.md + references/engines.md + scripts/search.py docstring. **Issue 2**: S2 silent failure on non-200 — added `logger.warning(f"[S2 search] status={s} data={str(data)[:200]}")` so 429/500 errors are visible. **Issue 3**: PubMed MeSH query syntax — **critical insight**: "Wilson Disease" is an ENTRY TERM (not main MeSH heading); main heading is "Hepatolenticular Degeneration". Verified live: plain query 36,857 papers (mostly author-surname Wilson), `"Wilson Disease"[MeSH Terms]` 0 papers (entry term not in main MeSH), `"Hepatolenticular Degeneration"[MeSH Terms]` 6,735 papers (relevant). Added comprehensive MeSH section to references/engines.md with the main-term-vs-entry-term gotcha + recommended fallback query. **Issue 4**: arXiv year post-filter — added `r.published.year` post-filter in `search_arxiv` (same pattern as PubMed) to handle arXiv's soft-relaxed API filter. E2E verified: 10 results all year >= 2024. **Tests**: new `_test_runner.py` with 4 sections, all PASS. Pre-existing 30 skill tests still PASS. **Version discipline**: MINOR (MeSH doc + S2 logging are new public-facing capabilities). AMiner multi-word query split (Issue 5 from user feedback) NOT fixed here — deferred to future release. |
| v3.9.25.0 | local 2026-08-27 (pending push) | **MINOR: AMiner multi-word query fix (Issue 5)**. User reported AMiner "only matches 'disease'" — "Wilson Disease" returned 36k+ irrelevant papers (AD, cardiovascular). Root cause: AMiner's basic `/paper/search` API only does exact title match, doesn't support multi-word. **Fix in 4 parts**: **(1) `search_aminer_pro()`** using `/paper/search/pro` with `keyword` param (¥0.01/call, native multi-word support, multi-field filters). **(2) `_title_relevance_score()`** — 0.0-1.0 scoring of title-query overlap (phrase bonus + Jaccard), used to re-rank basic results so "Wilson Disease" papers surface above "Wilson" author papers. **(3) `--aminer-mode` flag** in `pa search`: `auto` (default: pro for multi-word, basic for single-word), `pro` (always Pro), `basic` (only free). **(4) `search_aminer()` refactored** to support 3 modes; legacy behavior preserved in `_search_aminer_basic()`. New result fields: `match_type` (`pro_keyword` / `phrase:full` / `phrase:<word>` / `basic_fallback`), `title_relevance`. **E2E verified (2026-08-27)**: AMiner basic for "Wilson Disease" returns 20 results, top 5 ALL Wilson Disease papers (rel=1.0 each, e.g., "Wilson's Disease—Genetic Puzzles with Diagnostic Implications", "Structure of the Wilson Disease Copper Transporter ATP7B"). AMiner auto uses Pro (¥0.01), also returns 20 relevant. AMiner single-word "Wilson's" correctly mixes Wilson Disease with Wilson loops (legitimate). **Tests**: 12/12 PASS in `test_output/_test_v3_9_25_0_aminer.py` (relevance scoring + signatures + CLI flag). Pre-existing 29 skill tests still PASS. **Cost**: ~0.01 CNY per multi-word search, trivial. **Version discipline**: MINOR (new public surface: `search_aminer_pro()` + `--aminer-mode` flag + `match_type`/`title_relevance` result fields + Pro API capability). |
| v3.9.25.1 | local 2026-08-27 (pending push) | **PATCH: v3.9.25.0 packaging cleanup (4 user-reported issues)**. User installed v3.9.25.0 and ran the Wilson disease end-to-end test successfully. All 8 engines worked, PubMed MeSH returned relevant results, AMiner returned Wilson Disease papers, arXiv noise dropped from 5 → 1, $paper-agent skill installed cleanly. But 4 packaging inconsistencies: (1) SKILL.md frontmatter `version: 3.9.24.0` (should be 3.9.25.0); (2) SKILL.md description had 15-line duplicate v3.9.23.0 block with broken text `).rch for academic papers by` and listing "7 engines" (we have 8 since v3.9.12.0); (3) SKILL.md `### scripts/search.py — Search 7 engines` heading was leftover from v3.9.23.0; (4) `scripts/version.py` hardcoded `skill_version: "3.9.23.0"`; (5) Chinese text mojibake in 6 .py docstrings + 1 SKILL.md line (`鈥?` instead of `—`); (6) `scripts/search.py` L14 example had mojibake Chinese. Fix: bumped frontmatter + version.py, removed duplicate, replaced mojibake with proper UTF-8 + em-dash, stripped BOMs from 7 .py files, added `test_output/_test_v3_9_25_1_packaging.py` (8 tests catching all 4 issues going forward). **Tests**: 50/50 PASS (29 v3.9.23 + 12 v3.9.25 + 8 v3.9.25.1 packaging + 1 runtime). |
| v3.9.26.0 | local 2026-08-27 (pending push) | **MINOR: 4 `pa fetch-batch` bug fixes** (from Wilson disease batch download test, 2 DOIs, 53.7s, 2/2 success, 1.19MB total). **Bug 1**: TWO `fetch-batch` Click commands collided (OLD `fetch_batch()` CNKI guide + NEW `fetch_batch()` PDF downloader) — old function used `@main.command()` which defaults to `fetch-batch`, same name as new. Skill's `scripts/fetch_batch.py` got "No such option '--out-dir'" because the OLD function won. **Fix**: renamed OLD function to `cnki-guide` with `@main.command(name="cnki-guide")`. **Bug 2**: First download `size_bytes = 0` in summary JSON because `r.get('size', 0)` instead of `r.get('size_bytes', 0)` (key name mismatch). **Fix**: changed key to `size_bytes` in 2 places (DOI + title fallback). **Bug 3**: `--skip-existing` counter wrong — 2nd run correctly skipped both PDFs but showed `n_success: 2, n_skipped: 0` because skip path sets `success=True` and orchestrator only checked `result.success`. **Fix**: check `result.error == 'skipped-existing'` BEFORE `n_success` increment. **Bug 4**: PMC `.xml` intermediate not cleaned up after JATS-to-PDF. **Fix**: added `--clean-xml` flag. **Tests**: 10/10 NEW in `_test_v3_9_26_0_fetch_batch.py` (TestClickCommandNames, TestSizeKeyFix, TestSkipCounterFix, TestCleanXmlOption). Total 60/60 PASS across all suites. **MINOR bump** (3.9.25.1 → 3.9.26.0) — new public surface: `cnki-guide` command + `clean_xml` parameter. **User-side**: `pip install --upgrade paper-agent`; Skill's `scripts/fetch_batch.py` now works. |
| v3.9.27.0 | local 2026-08-28 (pending push) | **MINOR: 2 remaining v3.9.26.0 issues** (from Wilson disease batch test). **Issue 1**: Skill wrapper still failed with "No such option '--out-dir'". Root cause: wrapper used `python -m pa_cli.cli fetch-batch` which loaded a stale/cached `pa_cli/cli.py` (containing the OLD `fetch_batch()` CNKI guide that lacks `--out-dir`). **Fix**: changed to `python -m pa_cli` (documented package entry point via `pa_cli/__main__.py` which does `from .cli import main`). Always uses fresh editable-install code. **Issue 2**: PMC jats_pdf `size_bytes` still = 0 because v3.9.26.0 fix used `r.get('size_bytes', 0)` but `_pmc_jats_to_pdf` returns `"size"` not `"size_bytes"`. **Fix**: fallback chain `r.get("size_bytes") or r.get("size") or stat(file).st_size` — works for ALL channels. **Bonus**: Skill wrapper now exposes `--clean-xml` flag. **Tests**: 8/8 NEW in `_test_v3_9_27_0_skill_wrapper.py` (TestSkillWrapperEntry + TestSizeFallback + TestFetchBatchFetchStillWorks). Total 67/67 PASS. **MINOR bump** (3.9.26.0 → 3.9.27.0) — new public surface: `--clean-xml` wrapper flag + entry point change. **User-side**: `pip install --upgrade paper-agent` + re-pull skill; wrapper now works end-to-end with `--clean-xml`. |
| v3.9.28.0 | local 2026-08-28 (pending push) | **MINOR: Skill wrapper relative path bug** (last v3.9.27.0 issue). User tested v3.9.27.0 and confirmed core fixes (file sizes, skip counter, --clean-xml) all worked. But found relative paths (e.g. `refs.bib`) failed because the wrapper's `subprocess.run(cmd, cwd=str(pa_root))` changes subprocess CWD to the pa_cli repo dir, so a relative path was interpreted against `pa_root` instead of the user's invocation CWD. **Fix**: resolve user-provided paths to absolute via `Path(...).resolve()` BEFORE constructing the subprocess cmd. `Path.cwd()` is the user's invocation CWD, so relative paths get resolved against that. **Tests**: 6/6 NEW in `_test_v3_9_28_0_path_resolve.py` (TestPathResolution: 4 path types + line order check + cwd check). Total 73/73 PASS. **MINOR bump** (3.9.27.0 → 3.9.28.0) — user explicitly said "暂不替换正式版本" until this fix ships. **User-side**: `pip install --upgrade paper-agent` + re-pull skill. Now fully functional with relative paths from the user's working directory. |
...
## Competitor coupling 2026-08-18 (InstSci awareness + v3.9.14 backlog)

**Source**: User noticed two related open-source projects (`Rimagination/instsci` 288⭐ + `deathcats4/instsci-workflow` 52⭐, both MIT) open-sourced this week. Both focus on **Open Access first + browser-backed institutional access** for closed papers. User uses Codex to drive these as AI-agent-friendly tools (MCP server + stdio).
...
- paper-agent philosophy: **8-engine aggregation + gray-area sci-hub/CNKI fallback**
- InstSci has: 10+ publisher-specific workflows, MCP server (`instsci-mcp`), CloakBrowser persistent profile, Zotero sync, jobs/tail/resume task system
- paper-agent has: 8 search engines (3 more), LTR rerank, MoE router, sample pool, clinicaltrials.gov, Chinese (aminer + CNKI), 4-verifier security audit, AGPL-3.0 + No-AI-Training

### Action items for paper-agent v3.9.14

These 3 items are **accepted** (pass Global Rule, no user-maintenance tax, pure local code):

- **[P2-15] `pa jobs` status/tail/resume subcommand** — `pa_cli/batch_fetch.py` (existing, v3.9.10.7) currently has `--skip-existing` resume but no live status / log tail / interrupted-job resume. Add `pa jobs` Click group with 4 subcommands (`status`, `tail <job_id>`, `resume <job_id>`, `list`). Backed by a new `pa_cli/jobs.py` (~150 LOC) + `~/.paper-agent/jobs/` dir (manifest JSON per job). Zero gray-area exposure, zero new infra. Effort: ~2h. ⭐⭐⭐ **HIGHEST VALUE OF THE 3**.
...
**SHIPPED in v3.9.17.0** ([P3-28.1]): `pa search-and-import` ships with the full 7-step workflow (search → fetch → bucket → push → project create → add items → master note). 21 new tests pass. The master note auto-contains a markdown table of downloaded + failed (per-key, per-DOI, per-error). One hint line at end of successful run tells user the corresponding `pa obsidian project` commands to run manually for the Obsidian side (cross-linking by name convention).

**Still deferred to v3.9.17.1+** ([P3-29.1]):
- `--with-obsidian` flag for `pa search-and-import` (one-line sync to Obsidian)
- Auto-create matching Obsidian project when Zotero project is created
- Auto-append Zotero master note URL to Obsidian `index.md` (bidirectional link)

### External project tracker (for awareness only)

Future readers: these are NOT paper-agent dependencies, but they occupy the same problem space. Periodically check if they have releases worth coupling into our docs (not code).

- **Repo**: `Rimagination/instsci`
  - **GitHub**: https://github.com/Rimagination/instsci
  - **License**: MIT
  - **Stars** (at coupling): 288 (20 forks)
  - **What it is**: OA-first + browser-backed institutional access for academic papers
  - **MCP**: `instsci-mcp` (stdio JSON-RPC, agent-friendly)
  - **Coupling**: complementary (different philosophy). Docs cross-link via [P3-27]
  - **Last checked**: 2026-08-18
- **Repo**: `deathcats4/instsci-workflow`
...
- **`[P0-15] pa mcp serve` 补全 fetch 工具** — Round 14 / 2026-08-18 coupling: Codex / Claude Code can drive `pa search` via public `paper-search-mcp` (good) but cannot drive `pa fetch` (gap). Add a thin in-repo `pa_cli/mcp_fetch.py` (~30 LOC) that exposes `pa_fetch(doi, prefer, use_cache)` + `pa_batch_fetch(dois[], output_dir)` over stdio JSON-RPC. NO new server maintenance, NO new schemas beyond 2 simple JSON Schemas. **Solves**: agent can drive paper-agent fetch without SSH wrapper. **Risk**: very low (reuses existing CLI functions; same trust boundary as CLI invocation; can be disabled by simply not installing the MCP config).
  ...
  - **Acceptance criteria**:
    - `python -m pa_cli mcp-fetch` runs as stdio JSON-RPC server
    - 2 tools: `pa_fetch`, `pa_batch_fetch`
    - Both return existing fetch result dict (`saved_as`, `via_channel`, `cache_hit`, `error/handoff`)
...
**v3.9.17 backlog status** (post release):
- ✅ [P3-28.1] `pa search-and-import` (closes research workflow loop)
- **All Zotero items shipped** (3 collections + 1 read + 1 push + 1 search + 1 sync + 1 project + 1 import = 9 Zotero-touching commands). Only [P2-17.1] PDF upload + [P3-29.1] Obsidian auto-sync still deferred.

**v3.9.17.1 backlog status** (post release):
- ✅ [P2-17.1] PDF upload in `pa zotero push` (linked_file + imported_file modes via pyzotero API)
- **All Zotero items shipped + 100% complete** (metadata + PDFs). Only [P3-29.1] Obsidian auto-sync still deferred.

**v3.9.17.2 backlog status** (post release):
- ✅ [P3-29.1] `--with-obsidian` flag (closes full Zotero + Obsidian bidirectional loop)
- **No more deferred items**. Both v3.9.16 Zotero/Obsidian foundation + v3.9.17 orchestrator + v3.9.17.1 PDF upload + v3.9.17.2 Obsidian sync are all shipped. Future work would be new feature surface (P0/P1/P2/...).

**v3.9.18.0 backlog status** (post release):
- ✅ [P3-28.2] `pa zotero-project pull` + `pa zotero-project export-bib` (closes full Zotero <-> local pa project round-trip; the v3.9.15.0 `pa zotero push` was the other half of the round-trip)
- **Round-trip complete**: Zotero collection → local pa project (pull, v3.9.18.0) + local pa project → Zotero (push, v3.9.15.0) + Zotero collection → .bib (export-bib, v3.9.18.0). All 3 directions of Zotero ↔ filesystem now supported. Future work would be new feature surface (P0/P1/P2/...).

**v3.9.19.0 backlog status** (post release):
- ✅ [P3-28.3] `pa zotero-project diff` + `pa zotero-project sync` (closes incremental Zotero → local sync; the workflow that makes the v3.9.18.0 round-trip useful in practice)
- **Sync complete**: full Zotero ↔ local workflow is now `pull` (full initial copy) + `diff` (show what changed) + `sync` (apply incrementally) + `push` (local → Zotero). All 4 directions of Zotero ↔ filesystem now supported incrementally. Future work would be new feature surface (per-item update detection, 2-way reverse link, etc.) or non-Zotero work.

**v3.9.20.0 backlog status** (post release):
- ✅ [P2-19] `pa project corpus-stats` (per-project corpus analytics: n_papers, year, authors, venues)
- ✅ [P2-19.1] PRISMA mermaid auto-embed in `pa search-and-import` master note (reuses P1-3)
- ✅ [P3-29.2] `pa obsidian daily-link` (backlink in `4-Daily/YYYY-MM-DD.md`, idempotent)
- ✅ [P3-28.4] per-item update detection in `pa zotero-project diff/sync` (Zotero version map in meta.json, baseline established on first sync)
- **4 features in 1 release**: corpus analytics + journal-ready PRISMA + daily-note backlinking + per-item edit detection. Future work: per-item field-level update (v3.9.20 detects updates but doesn't re-write Bibtex for them; that would need diff-then-merge at field level), or non-research-workflow features

**v3.9.20.1 backlog status** (post release):
- ✅ [fix-pmc-default] `pa fetch` default auto mode now includes `pmc` channel (was missing, auto mode was skipping PMC entirely)
- **Triggered by**: user feedback “看别的 对话里说 paper agent 的 fetch 没办法用了” (2026-08-21)
- **Discipline applied**: verified with real tests before fixing — user claim was OVERSTATED (arXiv + sci-hub still worked) but pointed to a real bug (PMC papers failed in default mode)
- **Lesson**: any new channel added to `pa_cli/fetch.py` must also be added to the default `--channels` list in `pa_cli/cli.py`. Otherwise it’s dead code (only `--prefer X` explicit users get it).
- **Tradeoff** that was kept (later fixed in v3.9.21.0): PMC channel returned JATS XML saved with `.pdf` extension; real PDF required `scihub` or `unpaywall`

**v3.9.21.0 backlog status** (post release):
- ✅ [P3-30] `pa_cli/jats_to_pdf.py` — JATS XML → real PDF via Playwright Chromium (closes the v3.9.20.1 tradeoff)
- ✅ [P3-30.1] `--prefer pmc-pdf` CLI option (forces PMC + jats_to_pdf, skip Europe PMC, always returns real PDF)
- ✅ [P3-30.2] `fetch_pmc_doi` auto-fallback to jats_to_pdf when Europe PMC PDF render fails (most common case)
- **Key fix during dev**: figure URL pattern is `https://www.ncbi.nlm.nih.gov/pmc/articles/instance/{pmcid}/bin/<filename>`, NOT `/articles/PMC{id}/bin/...` (which 404s). _render_figure now accepts pmcid param and resolves relative hrefs
- **Why Playwright over weasyprint**: weasyprint needs GTK native libs (missing); reportlab/fpdf2/xhtml2pdf not in deps; Playwright Chromium already a dep for CNKI HTML rendering (v3.9.20)
- **Future work**:
  - [P3-30.3] Cache JATS XML in `~/.paper-agent/jats_cache/` keyed by PMCID (avoid re-EFetch for repeat fetches, ~10s savings per repeat)
  - [P3-30.4] Optional: extract figure captions to alt text for accessibility (currently uses caption[:100] truncated)
  - [P3-30.5] XSL-FO XML→PDF alternative (more accurate, but requires fop install; Playwright is good enough for now)
...
- ✅ **[P2-17.1] SHIPPED in v3.9.17.1** (see above) — `pa_cli.zotero_api.upload_pdfs()` supports both `linked_file` (default, no quota usage) and `imported_file` (uses Zotero storage quota ~300MB free). 16 new tests pass. Bug fix: `list_collection_notes()` was missing `return out` in v3.9.16.0; fixed in same commit. Encoding cleanup: truncated UTF-8 em-dash sequences in 4+ pa_cli files replaced with ASCII `--`.
