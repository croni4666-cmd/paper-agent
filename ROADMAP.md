...
## Skill capability repairs (2026-09-10)

- Crossref abstract/date integrity merged in PR #56; release pending.
- Implemented locally: source-stage diagnostics, optional-cache deadline isolation, caller-relative output and UTF-8 handling. A native arXiv download passes; broad full-text reliability remains unproven.
- Next: test other public full-text channels against fixed positive and failure fixtures, then assess ordering/budgets from traces. Keep material-update lineage, extraction context and appraisal evidence review in the companion skills; use topics only as test inputs.
